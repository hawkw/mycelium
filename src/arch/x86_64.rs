...
use hal_core::{boot::BootInfo, mem, PAddr, VAddr, Address};
use hal_x86_64::{cpu, interrupt::Registers as X64Registers, serial, vga, framebuffer::{Framebuffer, self}};
pub use hal_x86_64::{interrupt, mm, NAME};
use mycelium_util::{trace, sync::{spin, InitOnce}};
...
impl BootInfo for RustbootBootInfo {
    ...
    fn framebuffer(&self) -> Option<Self::Framebuffer> {
        ...
        Some(FRAMEBUFFER.get_or_else(move || {
